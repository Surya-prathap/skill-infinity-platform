...
import com.skillinfinity.review.entity.CompletedSession;
...
public class ExternalEventConsumer {
    ...
    @RabbitListener(queues = "${review.user.registered.queue:review.user.registered.queue}")
    public void handleUserRegisteredEvent(Map<String, Object> event) {
        log.info("Received user registered event: {}", event.get("eventId"));
    }

    @RabbitListener(queues = "${review.mentor.registered.queue:review.mentor.registered.queue}")
